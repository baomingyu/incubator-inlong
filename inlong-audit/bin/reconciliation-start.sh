...
SERVER_JAR=$(ls -lt "${BASE_DIR}"/lib |grep audit-reconciliation | head -2 | tail -1 | awk '{print $NF}')

check_server_is_running

echo $BASE_DIR/conf/reco

if [ $PID -gt 0 ]; then
  echo "server is already running ${PID}."
else
  nohup "$JAVA" $JAVA_OPTS -Daudit.log.path="$LOG_DIR" -Dloader.path="$BASE_DIR/conf,$BASE_DIR/lib/" -jar "$BASE_DIR/lib/$SERVER_JAR"  1>"${LOG_DIR}"/start.log  2>"${LOG_DIR}"/start-error.log &
  PID=$!
  if [ "$PID" -gt 0 ]; then
    echo -n "$PID" > "$PID_FILE"
